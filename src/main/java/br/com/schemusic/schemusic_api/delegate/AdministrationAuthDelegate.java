...
@Component
public class AdministrationAuthDelegate {

    private final AuthBusiness authBusiness;

    public AdministrationAuthDelegate(AuthBusiness authBusiness) {
        this.authBusiness = authBusiness;
    }

    public ResponseEntity<?> login(LoginRequestBean request) {
        try {
            LoginResponseBean response = authBusiness.loginAdministrador(request.email(), request.senha());
